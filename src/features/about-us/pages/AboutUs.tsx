    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white text-[#1A1A1A] font-sans selection:bg-[#67AEFF] selection:text-white overflow-x-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-12 md:gap-16 items-center">
          <div className="space-y-6 md:space-y-8 order-2 lg:order-1 animate-slideUp">
            ...
            <p className="text-sm md:text-base text-slate-500 leading-relaxed max-w-lg border-l-4 border-blue-100 pl-5">
              SkinNavi is an AI-powered universe designed to decode your skin's unique language. 
              We bridge the gap between complex dermatology and daily self-care.
            </p>
            <ul className="space-y-3 pt-2">
              {['Accurate AI Analysis', 'Personalized Routines', 'Visual Progress Tracking'].map((item, idx) => (
                <li key={item} className="flex items-center gap-3 text-sm font-semibold text-slate-700 animate-fadeIn" style={{ animationDelay: `${idx * 0.2}s` }}>
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-6 relative order-1 lg:order-2 animate-fadeIn">
            <div className="space-y-4 md:space-y-6">
              <div className="aspect-[4/5] overflow-hidden rounded-3xl shadow-xl border border-white">
                 <img src="https://i.pinimg.com/736x/2b/61/2d/2b612dab42da608746037483b05f269d.jpg" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" alt="Skincare" />
              </div>
              <div className="bg-blue-500 p-6 md:p-8 text-white rounded-[2rem] text-center shadow-blue-200 shadow-lg animate-slideUp">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest block">Precision AI</span>
              </div>
            </div>
            <div className="pt-8 md:pt-12 space-y-4 md:space-y-6">
              <div className="bg-slate-900 p-6 md:p-8 text-white rounded-[2rem] text-center shadow-slate-300 shadow-lg animate-slideUp">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest block">User First</span>
              </div>
              <div className="aspect-[4/5] overflow-hidden rounded-3xl shadow-xl border border-white">
                <img src="https://i.pinimg.com/736x/cb/2b/a5/cb2ba532cc07da737b71f4e15290d691.jpg" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" alt="Routine" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-blue-50/50 py-16 md:py-24 px-4 md:px-16 rounded-[2.5rem] md:rounded-[3.5rem] mx-4 md:mx-12 mb-10 shadow-inner border border-blue-100/50 overflow-hidden animate-slideUp">
        <div className="max-w-[1400px] mx-auto">
          <div className="text-center mb-12 md:mb-16 space-y-4">
            <h3 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight">The Minds Behind SkinNavi</h3>
            <div className="w-12 h-1 bg-blue-400 mx-auto rounded-full"></div>
            <p className="md:hidden text-[10px] text-slate-400 uppercase font-bold tracking-widest">Swipe to explore →</p>
          </div>
          ...
          <div className="flex md:grid md:grid-cols-4 gap-6 overflow-x-auto pb-8 md:pb-0 scrollbar-hide snap-x snap-mandatory px-2">
            {teamMembers.map((member, index) => (
              <div key={index} className="min-w-[75vw] sm:min-w-[45vw] md:min-w-0 snap-center group">
                <Card className="border-none bg-white rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden">
                  <CardContent className="p-0 text-center lg:text-left">
                    ...
                    <div className="p-6">
                      <h4 className={`font-bold text-slate-900 group-hover:text-blue-500 transition-colors leading-snug break-words ${member.name.length > 20 ? 'text-[13px] md:text-[15.5px]' : 'text-sm md:text-base'}`}>
                        {member.name}
                      </h4>
      ...
      <section className="container mx-auto px-6 md:px-16 py-12 md:py-20">
        <div className="flex md:grid md:grid-cols-3 gap-6 overflow-x-auto pb-10 md:pb-20 scrollbar-hide snap-x snap-mandatory px-2">
          {[
            { icon: Fingerprint, title: "AI Skin Scanning", desc: "Advanced computer vision analyzes facial markers with clinical precision." },
            { icon: Layers, title: "Smart Tracking", desc: "Visualize your progress over time with data-driven history." },
            { icon: Zap, title: "Expert Routine", desc: "Personalized product suggestions tailored to your unique biology." }
          ].map((tech, i) => (
            <div key={i} className={`min-w-[85vw] md:min-w-0 snap-center group p-8 md:p-10 rounded-[2.5rem] bg-white border border-blue-50 hover:border-blue-400 hover:shadow-2xl transition-all duration-500 animate-slideUp`} style={{ animationDelay: `${i * 0.2}s` }}>
              ...
              <h4 className="text-xl font-bold mb-4 group-hover:text-blue-500 transition-colors tracking-tight">{tech.title}</h4>
              <p className="text-sm text-slate-400 leading-relaxed font-medium group-hover:text-slate-600 transition-colors">{tech.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="relative group cursor-pointer border border-blue-100 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-16 flex flex-col lg:flex-row items-center justify-between gap-10 hover:shadow-2xl hover:border-blue-200 transition-all duration-500 animate-fadeIn bg-white">
          <div className="max-w-xl space-y-6 text-center lg:text-left">
            <Badge variant="outline" className="border-blue-100 text-blue-400 group-hover:text-blue-500 group-hover:border-blue-500 px-4 py-1 rounded-full uppercase tracking-widest text-[10px] font-bold transition-colors">
              Integrated Marketplace
            </Badge>
            <h3 className="text-3xl md:text-5xl font-bold tracking-tighter leading-[1] text-slate-900">
              Trusted Care, <br />
              <span className="text-slate-200 group-hover:text-blue-400 transition-colors duration-700">One Click Away.</span>
            </h3>
            <p className="text-slate-400 text-sm md:text-base font-medium leading-relaxed group-hover:text-slate-500 transition-colors">
              Reliable products recommended specifically for your skin type, curated by our AI engine.
            </p>
          </div>
