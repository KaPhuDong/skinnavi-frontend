...
const EditProductModal: React.FC<{ 
  ...
  useEffect(() => { if (product) setFormData(product); }, [product, isOpen]);

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData) return;

    if (!formData.name?.trim()) return toast({ title: "Required Field", description: "Please enter product name.", variant: "destructive" });
    if (!formData.usage_role) return toast({ title: "Required Field", description: "Please select a category.", variant: "destructive" });
    if (!formData.display_price) return toast({ title: "Required Field", description: "Please enter price.", variant: "destructive" });
    if (!formData.image_url?.trim()) return toast({ title: "Required Field", description: "Image URL cannot be empty.", variant: "destructive" });
    if (!formData.affiliate_url?.trim()) return toast({ title: "Required Field", description: "Affiliate Link is required.", variant: "destructive" });
  ...
  const inputClass = "w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:ring-4 focus:ring-[#67AEFF]/10 focus:border-[#67AEFF] transition-all bg-gray-50/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none font-['Poppins']";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-[1.5rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200 border border-gray-50">
        <div className="px-6 pt-6 pb-2 flex justify-between items-center">
          <h3 className="text-xl font-bold text-gray-900 tracking-tight font-['Poppins']">Edit Product</h3>
        ...
        <form onSubmit={handleSave} noValidate className="p-6 space-y-4 text-left">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className={inputClass} value={formData.usage_role} onChange={(e) => setFormData({...formData, usage_role: e.target.value})}>
          ...
          <div><label className={labelClass}>Affiliate Link</label><input type="text" className={inputClass} value={formData.affiliate_url} onChange={(e) => setFormData({...formData, affiliate_url: e.target.value})} /></div>
...
const SingleProductTable: React.FC<any> = ({ itemsPerPage, currentPage, refreshKey, onTotalPagesChange, onEdit, onDelete }) => {
  ...
  const handleUpdate = async (updated: Product) => {
    try {
      ...
      toast({ title: 'Update Failed', description: error?.response?.data?.message || 'Error occurred.', variant: 'destructive' });
      throw error;
    }
  };
  ...
  if (loading) return <div className="p-20 text-center text-gray-400 font-['Poppins'] animate-pulse font-medium">Loading...</div>;
        ...
        <thead>
          <tr className="border-b border-gray-100 bg-gray-50/60 text-gray-400">
            {['#', 'Category', 'Product Details', 'Price', 'Source', 'Actions'].map((h, i) => (
              <th key={h} className={`px-4 py-5 text-[10px] font-bold uppercase tracking-widest ${i === 5 && 'text-right'}`}>{h}</th>
            ))}
          </tr>
        </thead>
          ...
          {data.map((p, i) => {
            ...
            return (
              <tr key={p.id} className="group hover:bg-slate-50/80 transition-colors">
                <td className="px-4 py-4 text-xs text-gray-300 font-mono text-center w-10">{String(displayIndex).padStart(2, '0')}</td>
                <td className="px-4 py-4 whitespace-nowrap">
                    <span style={{ backgroundColor: style.bg, color: style.text }} className="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase border border-black/5 tracking-wider">{p.usage_role}</span>
                </td>
                ...
                <td className="px-4 py-4">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setEditTarget(p)} className="p-2 bg-white border border-blue-50 text-[#67AEFF] rounded-lg shadow-sm hover:bg-[#67AEFF] hover:text-white transition-all"><Edit3 size={14} /></button>
                    <button onClick={() => setDeleteTarget(p)} className="p-2 bg-white border border-red-50 text-red-500 rounded-lg shadow-sm hover:bg-red-500 hover:text-white transition-all"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            );
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white p-8 rounded-[1.5rem] max-w-sm w-full text-center shadow-2xl animate-in zoom-in border border-gray-50">
            <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-5"><AlertTriangle size={32} /></div>
            <h3 className="text-xl font-bold text-gray-900 mb-2 tracking-tight">Confirm Delete</h3>
            <p className="text-sm text-gray-500 mb-8 leading-relaxed font-medium">Remove <span className="font-semibold text-gray-800">"{deleteTarget.name}"</span>?</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)} className="flex-1 py-3.5 bg-gray-50 text-gray-600 font-bold rounded-xl hover:bg-gray-100 transition-all">Cancel</button>
              <button onClick={finalDelete} className="flex-1 py-3.5 bg-red-600 text-white font-bold rounded-xl shadow-lg shadow-red-100 hover:bg-red-700 transition-all">Delete Now</button>
            </div>
          </div>
        </div>
      )}
